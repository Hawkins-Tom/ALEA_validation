    ...
    for fi, fpath in enumerate(data_files[tracker]):
        ...
        for exp_name in start_msg.keys():
            ...
            for i in range(len(data)):
                ...
                # There are undetected blinks in the EyeLink data, so we'll have to
                # manually detect and exclude them. We're also including a 50 ms
                ...
                # TODO: Recompute x and y from pixels to degrees of visual angle.
            ...
            if exp_name == "calibration":
                ...
                for i in range(len(data)):
                    ...
                    d_target = numpy.sqrt((x-target_x)**2 + (y-target_y)**2)
                    # TODO: Kick out samples that are obviously too far? (I.e. someone
            ...
            # FREEVIEWING EXPERIMENT
                ...
                for i in range(0, len(data)):
                    ...
                    if baseline is not None:
                        ...
                        # a NaN (mean of empty slice). This means we can't 
                        # correct.
                ...
                for i in range(len(data)):
                    ...
                    # # TODO: DOES THIS WORK? 
                    # if p_too_far > 0.5:
                    #     print(("\tParticipant moved from fixation " \
                    #        + "{}, trial {}!").format(exp_name, i))
                    #     continue
...
for tracker in trackers:
    ...
    for px,py in CALIB_DOTS_PX:
        ...
        for phase_index in range(results["calibration"]["x"].shape[2]):
            ...
            ax.scatter(x_data, y_data, label ='Test point {}'.format(phase_index +1), alpha = 0.1, s = 1, c = colour)
...
#initialise dict for mean and standard deviation for fix nr
prop_p_m = {}
prop_p_sd = {}
...
for aoi_name in aoi_features:
    ...
    prop_p_sd[aoi_name] = numpy.nanstd(results["aoi"]["proportion"][aoi_name], axis=0)
                ...
                # TODO: Count the number of results that is not a NaN.
                # fix_nr_n = numpy.sum(numpy.invert(numpy.isnan(results["aoi"]["fix_nr"])), \
                #    axis=0)
                    
                # TODO: Compute the standard error of the mean.
                # fix_nr_sem = fix_nr_m_sd / numpy.sqrt(fix_nr_n)
...
for aoi_name in aoi_features:
    ...
    dwell_p_sd[aoi_name] = numpy.nanstd(results["aoi"]["dwell"][aoi_name], axis=0)    
            ...
            # TODO: Count the number of results that is not a NaN.
            # dwell_n = numpy.sum(numpy.invert(numpy.isnan(results["aoi"]["dwell"])), \
            #    axis=0)
            
            # TODO: Compute the standard error of the mean.
            # dwell_sem = dwell_sd / numpy.sqrt(dwell_n)
...
for ti, tracker in enumerate(trackers):
    for img_name in image_names:
        ...
        for aoi_name in aoi_features:
            ...
            #fix_p_sem_value = fix_p_sem[aoi_name][ti, img_index]
            #fix_p_count = fix_n[ti, img_index]
            ...
            dwell_p_df = pandas.concat([dwell_p_df, pandas.DataFrame({
              ...
              #  "SEM Sample number": [dwell_sem_value],
            })], ignore_index=True)
...
for tracker in trackers:
    for aoi_image_name in image_names:
        ...
        # extent = [0, DISPSIZE[0], 0, DISPSIZE[1]]
       # extent = [x_si, x_ei, y_ei, y_si]
...
for i in range(att_p_tn_m.shape[0]):  # Iterate over the first dimension
    ...
    for j in range(att_p_tn_m.shape[1]):  # Iterate over the second dimension
        ...
        # if ti == 'Alea':
        #     mean = att_p_tn_m[i, j, 0:ei]
        #     std = att_p_tn_sd[i, j, 0:ei]
        # else:
        #     mean = att_p_tn_m[i, j, :]
        #     std = att_p_tn_sd[i, j, :]
...
for measure in calib_measures_analyse:
    ...
    #shape = list(results["calibration"][measure].shape)
    #calibration_lme = numpy.zeros(shape, \
    #    dtype=results["calibration"][measure].dtype)
    ...
    for i in range(n_calib_points):
        ...
        for j, tracker_type in enumerate(trackers):
            ...
            hoc_results[:,:,:] = (hoc_results[:,:,:] - numpy.nanmean(hoc_results[:,:,:]))/numpy.nanstd(hoc_results[:,:,:])

            pre_ = hoc_results[:,j,0]
            post_ = hoc_results[:,j,1]  
            
            #pre_ = results["calibration"][measure][:,j,0,i]
            #post_ = results["calibration"][measure][:,j,1,i]  
...
# Do one LME per outcome variable. We'll actually just focus on dwell and 
            ...
            for ppi, ppname in enumerate(ppnames):
                for ti, tracker_name in enumerate(trackers):
                    for aoi in aoi_names:
                        ...
                        f.write("\n" + ",".join(map(str, line)))
        # Load the data we just saved into a pandas dataframe. (Ew)
...
shape = results["aoi"]["x"].shape
n_participants = shape[0]
n_trackers = shape[1]
n_trials = shape[2]

# Track participants with more than 100 samples for one tracker
participants_with_more_than_400_samples = []
trials_w_400_plus = []

# Iterate over participants
for participant_index in range(n_participants):
    for trial_index in range(n_trials):
        # Sum the number of samples across all trackers for this participant
        total_samples = numpy.sum(numpy.invert\
            (numpy.isnan(results["aoi"]["x"]\
            [participant_index, 0, trial_index, :])))
        # Check if total samples exceed 400
        if total_samples > 400:
            print(total_samples)
            trials_w_400_plus.append(trial_index)
            participants_with_more_than_400_samples.append(participant_index)
            
samples = []
    
samples = numpy.sum(numpy.invert\
    (numpy.isnan(results["att_pupil"]["corrected"]\
    [14, 0, :, 43, :])))
